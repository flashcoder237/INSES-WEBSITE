-- =====================================================
-- SCHEMA SQL AMÉLIORÉ - SUPPORT MULTI-CENTRES (VERSION CORRIGÉE)
-- =====================================================
...
-- =====================================================
-- MODIFIER LA TABLE formations (avec vérification)
-- Ajouter la référence au centre
-- =====================================================
DO $$
BEGIN
    -- Ajouter center_id si elle n'existe pas
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'formations' AND column_name = 'center_id'
    ) THEN
        ALTER TABLE formations ADD COLUMN center_id UUID REFERENCES centers(id) ON DELETE CASCADE;
    END IF;

    -- Ajouter center_slug si elle n'existe pas
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'formations' AND column_name = 'center_slug'
    ) THEN
        ALTER TABLE formations ADD COLUMN center_slug VARCHAR(100);
    END IF;
END $$;
...
-- Informations spécifiques à chaque centre
-- =====================================================
...
-- =====================================================
-- TRIGGER pour updated_at (vérifier existence)
-- =====================================================
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'update_centers_updated_at'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'update_center_info_updated_at'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'update_center_values_updated_at'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'update_center_partners_updated_at'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'update_center_stats_updated_at'
    ) THEN
...
DROP POLICY IF EXISTS "Public can read active centers" ON centers;
DROP POLICY IF EXISTS "Authenticated users can manage centers" ON centers;
DROP POLICY IF EXISTS "Public can read center_info" ON center_info;
DROP POLICY IF EXISTS "Authenticated users can manage center_info" ON center_info;
DROP POLICY IF EXISTS "Public can read center_values" ON center_values;
DROP POLICY IF EXISTS "Authenticated users can manage center_values" ON center_values;
DROP POLICY IF EXISTS "Public can read center_partners" ON center_partners;
DROP POLICY IF EXISTS "Authenticated users can manage center_partners" ON center_partners;
DROP POLICY IF EXISTS "Public can read center_stats" ON center_stats;
DROP POLICY IF EXISTS "Authenticated users can manage center_stats" ON center_stats;
...
-- =====================================================
-- MIGRER about_values VERS center_values POUR INSES (si existe)
-- =====================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'about_values') THEN
...
-- =====================================================
-- MIGRER partners VERS center_partners POUR INSES (si existe)
-- =====================================================
DO $$
BEGIN
...
-- =====================================================
-- MIGRER stats VERS center_stats POUR INSES (si existe)
-- =====================================================
DO $$
BEGIN
...
-- =====================================================
-- FIN DU SCHEMA MULTI-CENTRES (VERSION CORRIGÉE)
-- =====================================================
